...
if(isset($_POST["emp_id"])) { 
    ...
    while ($row = mysqli_fetch_array($result)) {    
            ...
            <p>Status: <span id="pop-status">'.$row["status"].'</span></p>
            <p>Comment: <span id="pop-statuss">'.$row["comments"].'</span></p>';
            if($row["status"]!= 'Approved' && $row["status"]!= 'Rejected' && $row["comments"]!='Completed by works directory') {
...
if(isset($_POST["approve"])) {
    $query = "UPDATE complaint SET status = 'Approved', comments = 'Approved by hostel manager' WHERE id = '".$_POST["approve"]."'";
    mysqli_query($data, $query);
}

if(isset($_POST["reject"])) {
    $query = "UPDATE complaint SET status = 'Rejected', comments = 'Rejected by hostel manager' WHERE id = '".$_POST["reject"]."'";
    mysqli_query($data, $query);
}
?>